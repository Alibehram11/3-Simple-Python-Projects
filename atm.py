...
2-Kayıt ol
""")
yapmak = input("yapmak istediğiniz işlem: ")
while True:
    if yapmak == "1":
        ad_sorgu = input("Adınızı girin: ")
        soyad_sorgu = input("Soyadınızı girin: ")
        şifre_sorgu = input("Parolanızı girin: ")
        if ad_sorgu == a and şifre_sorgu == ş and soyad_sorgu == so:
            print("Giriş başarılı!")
            break
        else:
            print("Kullanıcı adı veya parola hatalı. Tekrar deneyin.")
    if yapmak == "2":
        kullanıcı = str(input("adınızı giriniz: "))
        kullanıcsoy = str(input("soy adınızı giriniz: "))
        kullanıcışife = int(input("şifre oluşturnuz: "))
        print("programa aktarılıyorsunuz.........")
        time.sleep(0.5)
        break
